...
# class CustomFuncGate(Func):
#     def __init__(self, distance, key1, key2=None):
#         if key2 is None:
#             key2 = key1
#         code = f"""
#             __device__ int main(const Item* item1, const Item* item2) {{
#                 float d = item1->{key1} - item2->{key2};
...
# class CustomExprGate(Expr):
#     def __init__(self, distance, key1, key2=None):
#         if key2 is None:
#             key2 = key1
#         code = f"""
#             (item1->{key1} - item2->{key2}) * (item1->{key1} - item2->{key2}) < {distance} * {distance}
#         """
#         super().__init__(code)
